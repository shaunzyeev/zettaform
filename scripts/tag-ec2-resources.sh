...
INSTANCEIDS=`aws --profile connect ec2 describe-instances \
...
do  
 	...
 	NAME=`aws --profile connect ec2 describe-instances \
	 --instance-id $I  \
 	...
 	VOLUMEID=`aws --profile connect ec2 describe-instances \
	 --instance-id $I \
    ...
    if [ ! -z $NONROOTVOL ]
    	then
 		 ...
 		 aws --profile connect ec2 create-tags \
		  --resources $NONROOTVOL \
		  --tags Key=Name,Value="dev-sdf-$NAME"
 	fi
